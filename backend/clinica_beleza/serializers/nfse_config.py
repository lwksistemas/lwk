"""Serializer de configuração NFS-e da Clínica da Beleza."""
from rest_framework import serializers

from ..models import ClinicaBelezaNFSeConfig


class ClinicaBelezaNFSeConfigSerializer(serializers.ModelSerializer):
    ...
    def get_asaas_webhook_url(self, obj):
        ...
        if not loja:
            return ""
        slug = (getattr(loja, "atalho", None) or getattr(loja, "slug", "") or "").strip()
        if not slug:
            return ""
        ...
        return f"{base}/api/clinica-beleza/webhooks/asaas/{slug}/"

    def get_asaas_webhook_token_configured(self, obj):
        return bool((getattr(obj, "asaas_webhook_token", None) or "").strip())
