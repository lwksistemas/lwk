...
interface AssinarConsentimentoFormProps {
  termo: TermoConsentimentoData;
  erro: string;
  assinando: boolean;
  baixandoPdf: boolean;
...
function AssinarConsentimentoForm({
  termo,
  erro,
  assinando,
  baixandoPdf,
...
export function AssinarConsentimentoPageContent({ tokenRaw }: { tokenRaw: string }) {
  const state = useAssinarConsentimento(tokenRaw);

  if (state.loading) return <AssinarConsentimentoLoading />;
  if (state.sucesso) return <AssinarConsentimentoSucesso proximoStatus={state.proximoStatus} />;
  if (state.erro && !state.termo) return <AssinarConsentimentoErro erro={state.erro} />;
  if (!state.termo) return null;

  return (
    <AssinarConsentimentoForm
      termo={state.termo}
      erro={state.erro}
