...
def serialize_whatsapp_config(config, loja=None, *, sync_evolution=False):
    ...
    payload = {
        ...
        "mensagem_cobranca": (getattr(config, "mensagem_cobranca", None) or "").strip(),
...
def apply_whatsapp_config_patch(config, data):
    ...
    if "mensagem_cobranca" in data:
        config.mensagem_cobranca = (data.get("mensagem_cobranca") or "").strip()
        update_fields.append("mensagem_cobranca")
