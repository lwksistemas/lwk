...
// ============================================================================
// CONFIGURAÇÕES - SINGLE SOURCE OF TRUTH
// ============================================================================

// Configuração de status dos agendamentos
...
// Configuração padrão para status desconhecido
...
export default function DashboardCabeleireiro({ loja }: { loja: LojaInfo }) {
  ...
  const [userRole, setUserRole] = useState<UserRole>('administrador'); // Role do usuário logado
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && loja?.id) {
      ...
      // Por enquanto, assumir que o usuário logado é o dono (administrador)
      // TODO: Implementar endpoint /funcionarios/me/ para carregar role real
  ...
  // ============================================================================
  // CONFIGURAÇÃO DOS BOTÕES DE AÇÃO - DRY + ROLES
  // ============================================================================
  ...
  // Filtrar botões baseado nas permissões do role
  const actionButtons = filterActionsByRole(allActionButtons, userRole);

  // Configuração dos cards de estatísticas (filtrados por role)
  ...
  return (
    <div className="space-y-6 sm:space-y-8 px-2 sm:px-4 lg:px-8">
      ...
      {/* Estatísticas - Apenas se tiver permissão */}
      {statCards.length > 0 && (
...
// ============================================================================
// COMPONENTES AUXILIARES - SOLID (SINGLE RESPONSIBILITY)
// ============================================================================
...
// Componente de botão de ação rápida - Reutilizável
...
// Componente de card de agendamento - Reutilizável
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl 
      ...
      <div className="flex items-center gap-2 sm:gap-3 pl-13 sm:pl-0">
        ...
        {/* Status com dropdown - apenas se puder editar */}
        {canEdit ? (
        ...
        {/* Botão Excluir - apenas se puder excluir */}
        {canDelete && (
          <button
            onClick={(e) => {
              e.stopPropagation();
