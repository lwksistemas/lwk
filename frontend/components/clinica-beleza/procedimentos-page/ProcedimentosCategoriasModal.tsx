...
export function ProcedimentosCategoriasModal({ open, onClose, onChanged, lojaCtx }: Props) {
  const [categorias, setCategorias] = useState<ProcedimentoCategoriaItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(formatApiErrorBody(err) || "Não foi possível carregar categorias.");
      setCategorias([]);
    } finally {
      setLoading(false);
    }
  ...
  const startEdit = (cat: ProcedimentoCategoriaItem) => {
    setIsCreating(false);
    setEditingId(cat.id);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await ClinicaBelezaAPI.procedures.categorias.update(editingId, { nome, cor: formCor });
      } else {
        await ClinicaBelezaAPI.procedures.categorias.create({ nome, cor: formCor }, lojaCtx);
      }
      resetForm();
      await load();
      onChanged();
    } catch (err) {
      setError(formatApiErrorBody(err) || "Não foi possível salvar.");
    } finally {
      setSaving(false);
    }
  };

  const handleDelete = async (cat: ProcedimentoCategoriaItem) => {
    if (!confirm(`Excluir a categoria "${cat.nome}"? Procedimentos nesta categoria impedem a exclusão.`)) {
      return;
    }
    setSaving(true);
    setError(null);
    try {
      await ClinicaBelezaAPI.procedures.categorias.delete(cat.id);
      if (editingId === cat.id) resetForm();
      await load();
      onChanged();
    } catch (err) {
      setError(formatApiErrorBody(err) || "Não foi possível excluir.");
    } finally {
      setSaving(false);
    }
  };
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-xs text-gray-500">
                  {cat.procedimentos_count ?? 0} procedimento
                  {(cat.procedimentos_count ?? 0) !== 1 ? "s" : ""}
                </span>
              </span>
              <button
                type="button"
