...
type Hospede = { id: number; nome: string };
type Quarto = { id: number; numero: string; nome?: string | null };
type Tarifa = { id: number; nome: string; valor_diaria: string | number };
...
export default function HotelReservasPage() {
  ...
  const loadBase = useCallback(async () => {
    try {
      const [h, q, t] = await Promise.all([
        apiClient.get<Hospede[] | { results?: Hospede[] }>('/hotel/hospedes/'),
        apiClient.get<Quarto[] | { results?: Quarto[] }>('/hotel/quartos/'),
        apiClient.get<Tarifa[] | { results?: Tarifa[] }>('/hotel/tarifas/'),
      ]);
      ...
      // best-effort; reservas ainda podem abrir
    }
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await apiClient.get<Reserva[] | { results?: Reserva[] }>('/hotel/reservas/');
  ...
  const openEdit = (r: Reserva) => {
    setEditing(r);
    setForm({
      hospede: String(r.hospede ?? ''),
      quarto: String(r.quarto ?? ''),
      tarifa: r.tarifa ? String(r.tarifa) : '',
      data_checkin: r.data_checkin || '',
      data_checkout: r.data_checkout || '',
      adultos: Number(r.adultos ?? 2),
      criancas: Number(r.criancas ?? 0),
      canal: r.canal || '',
      status: r.status || 'pendente',
      valor_diaria: String(r.valor_diaria ?? ''),
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (editing) {
        await apiClient.put(`/hotel/reservas/${editing.id}/`, payload);
      } else {
        await apiClient.post(`/hotel/reservas/`, payload);
      }
      setModalOpen(false);
      setEditing(null);
      resetForm();
  ...
  const remove = async (r: Reserva) => {
    if (!confirm(`Excluir a reserva #${r.id}?`)) return;
    try {
      await apiClient.delete(`/hotel/reservas/${r.id}/`);
      await load();
    } catch (e: any) {
  ...
  const doCheckin = async (r: Reserva) => {
    try {
      await apiClient.post(`/hotel/reservas/${r.id}/checkin/`);
      await load();
    } catch (e: any) {
      alert(e?.response?.data?.detail || 'Erro no check-in.');
    }
  };

  const doCheckout = async (r: Reserva) => {
    try {
      await apiClient.post(`/hotel/reservas/${r.id}/checkout/`);
      await load();
    } catch (e: any) {
      alert(e?.response?.data?.detail || 'Erro no check-out.');
    }
  };
  ...
  const resumo = useMemo(() => {
    ...
    const checkin = items.filter((i) => i.status === 'checkin').length;
    const confirmadas = items.filter((i) => i.status === 'confirmada').length;
    return { total, checkin, confirmadas };
  }, [items]);
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Total: {resumo.total} • Confirmadas: {resumo.confirmadas} • Em hospedagem (check-in): {resumo.checkin}
          </p>
                ...
                <tr key={r.id} className="text-gray-900 dark:text-gray-100">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Button variant="outline" onClick={() => openEdit(r)} className="min-h-[36px]">
                        Editar
                      </Button>
                      {r.status !== 'checkin' && r.status !== 'checkout' && r.status !== 'cancelada' && (
                        <Button onClick={() => doCheckin(r)} className="min-h-[36px]">
                          Check-in
                        </Button>
                      )}
                      {r.status === 'checkin' && (
                        <Button onClick={() => doCheckout(r)} className="min-h-[36px]">
                          Check-out
                        </Button>
                      )}
                      <Button variant="destructive" onClick={() => remove(r)} className="min-h-[36px]">
                        Excluir
                      </Button>
                    </div>
                  </td>
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Check-in/check-out, hóspedes, quarto, canal e valores.</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <select
                ...
                value={form.hospede}
                onChange={(e) => setForm((f) => ({ ...f, hospede: e.target.value }))}
              >
              ...
              <Link href={`/loja/${slug}/hotel/hospedes`} className="text-xs text-sky-700 hover:underline">
                + cadastrar hóspede
              </Link>
            </div>
            ...
            <div className="space-y-2">
              ...
              <select
                className="w-full h-10 px-3 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100"
                value={form.tarifa}
                onChange={(e) => autofillTarifa(e.target.value)}
              >
            ...
            <div className="space-y-2">
              ...
              <Input type="date" value={form.data_checkin} onChange={(e) => setForm((f) => ({ ...f, data_checkin: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label>Check-out *</Label>
              <Input type="date" value={form.data_checkout} onChange={(e) => setForm((f) => ({ ...f, data_checkout: e.target.value }))} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <select
                className="w-full h-10 px-3 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100"
                value={form.status}
                onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as Reserva['status'] }))}
              >
            ...
            <div className="space-y-2">
              <Label>Adultos</Label>
              <Input type="number" min={0} value={form.adultos} onChange={(e) => setForm((f) => ({ ...f, adultos: Number(e.target.value || 0) }))} />
            </div>
            <div className="space-y-2">
              <Label>Crianças</Label>
              <Input type="number" min={0} value={form.criancas} onChange={(e) => setForm((f) => ({ ...f, criancas: Number(e.target.value || 0) }))} />
            </div>

            <div className="space-y-2">
              <Label>Canal</Label>
              <Input value={form.canal} onChange={(e) => setForm((f) => ({ ...f, canal: e.target.value }))} placeholder="Direto / Booking / Expedia" />
            </div>

            <div className="space-y-2">
              <Label>Valor diária (R$)</Label>
              <Input value={form.valor_diaria} onChange={(e) => setForm((f) => ({ ...f, valor_diaria: e.target.value }))} />
            </div>
            <div className="space-y-2">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              ...
              disabled={saving || !form.hospede || !form.quarto || !form.data_checkin || !form.data_checkout}
            >
