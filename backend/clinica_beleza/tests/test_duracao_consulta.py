...
class DuracaoConsultaTests(TestCase):
    def test_base_usa_profissional(self):
        prof = SimpleNamespace(tempo_consulta_minutos=45)
        self.assertEqual(tempo_consulta_base_minutos(professional=prof), 45)

    def test_base_usa_local_quando_profissional_sem_tempo(self):
        local = SimpleNamespace(tempo_consulta_minutos=40)
        self.assertEqual(tempo_consulta_base_minutos(local_atendimento=local), 40)

    def test_profissional_prevalece_sobre_local(self):
        prof = SimpleNamespace(tempo_consulta_minutos=50)
        local = SimpleNamespace(tempo_consulta_minutos=40)
        self.assertEqual(
            tempo_consulta_base_minutos(professional=prof, local_atendimento=local),
            50,
        )
