...
class UpdateEvolutionConnectionFromWebhookTest(SimpleTestCase):
    ...
    @patch('core.db_config.ensure_loja_database_config')
    ...
    def test_marca_desconectado_quando_state_close(
        self, mock_config_cls, mock_loja_cls, _ensure_db, mock_apply,
    ):
        loja = MagicMock()
        ...
        config = MagicMock()
        config.loja_id = 6
        mock_config_cls.objects.using.return_value.filter.return_value.first.return_value = config
