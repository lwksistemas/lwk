"""
Comando para adicionar coluna duracao_minutos em schemas que têm crm_vendas_atividade mas não têm a coluna.

Uso:
  python manage.py fix_duracao_minutos_column
  heroku run "cd backend && python manage.py fix_duracao_minutos_column" -a lwksistemas
"""
...
class Command(BaseCommand):
    help = 'Adiciona duracao_minutos em schemas com crm_vendas_atividade que não têm a coluna'
            ...
            with connection.cursor() as cursor:
                ...
                try:
                    if not all(c.isalnum() or c == '_' for c in schema):
                        ...
                        continue
                    cursor.execute(
                        'ALTER TABLE "{}".crm_vendas_atividade ADD COLUMN duracao_minutos INTEGER NOT NULL DEFAULT 60'.format(
                            schema.replace('"', '""')
                        )
                    )
