...
def criar_profissional_com_acesso(
    ...
    *,
    ...
    if existing_user:
        ...
        if ProfissionalUsuario.objects.using(default_db).filter(
            ...
            raise ProfessionalAccessError(
                'email',
                'Este e-mail já possui acesso a esta loja. Use outro ou cadastre sem "Criar acesso".',
            )
        # É proprietário de alguma loja
        if existing_user.lojas_owned.exists():
            raise ProfessionalAccessError(
                'email',
                'Já existe um usuário (proprietário de loja) com este e-mail. Use outro ou não marque "Criar acesso".',
            )
        ...
        try:
            ...
            raise ProfessionalAccessError(
                'email',
                'Este e-mail já possui acesso a esta loja ou há conflito de dados.',
            )
        ProfissionalUsuario.objects.using(default_db).create(
            user=user, loja=loja, professional_id=professional.id,
            perfil=perfil, precisa_trocar_senha=True,
    ...
    _enviar_email_senha(loja, email, name, senha_provisoria, perfil)
    return user


def _enviar_email_senha(loja, email, name, senha, perfil):
