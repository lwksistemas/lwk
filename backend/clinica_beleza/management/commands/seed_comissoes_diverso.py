...
class Command(BaseCommand):
    ...
    def _criar_atendimento(self, db, lid, dia_offset, hora, prof, paciente, local, procs, hoje, inicio_mes):
        from clinica_beleza.models import Appointment, AppointmentProcedure, Consulta, Payment
        ...
        with transaction.atomic(using=db):
            appointment = Appointment.objects.using(db).create(
                date=dt, status="COMPLETED", patient=paciente,
                professional=prof, procedure=primary, loja_id=lid,
            )
            for ordem, proc in enumerate(procs):
                AppointmentProcedure.objects.using(db).create(
                    appointment=appointment, procedure=proc, ordem=ordem, valor=proc.preco, loja_id=lid,
                )
            ...
            Consulta.objects.using(db).create(
                appointment=appointment, patient=paciente, professional=prof,
                procedure=primary, status="COMPLETED", data_inicio=dt,
                data_fim=dt + timedelta(hours=1), valor_consulta=valor_consulta,
                local_atendimento=local, loja_id=lid,
            )
            Payment.objects.using(db).create(
                appointment=appointment, amount=valor_total,
                payment_method=["PIX", "CREDIT_CARD", "DEBIT_CARD", "CASH"][dia_offset % 4],
                status="PAID", payment_date=dt + timedelta(minutes=20),
                comissao_percentual=0, comissao_valor=Decimal(0), loja_id=lid, notes=SEED_NOTE,
            )
        procs_txt = ", ".join(p.nome for p in procs)
        self.stdout.write(f'   \u2022 {data.strftime("%d/%m/%Y")} {hora} \u2014 {prof.nome} @ {local.nome}: {procs_txt} \u2014 total R$ {valor_total}')

    def _seed_profissionais(self, db, lid, locais, models):
        HorarioTrabalhoProfissional = models["HorarioTrabalhoProfissional"]
        Professional = models["Professional"]
        ProfessionalCommission = models["ProfessionalCommission"]
        profissionais = []
        self.stdout.write("\nProfissionais:")
        for nome, esp, email_base in PROFISSIONAIS_SEED_DATA:
            ...
            prof, _ = Professional.objects.using(db).update_or_create(
                ...
                defaults={"nome": nome, "especialidade": esp, "telefone": "(11) 99000-0000", "is_active": True, "comissao_percentual": Decimal(0)},
            )
            for dia in range(5):
                HorarioTrabalhoProfissional.objects.using(db).update_or_create(
                    professional=prof, dia_semana=dia, loja_id=lid,
                    defaults={"hora_entrada": datetime.strptime("08:00", "%H:%M").time(), "hora_saida": datetime.strptime("18:00", "%H:%M").time()},
                )
            profissionais.append(prof)
            self.stdout.write(f"   \u2022 {nome}")
        if len(profissionais) < 3:
            extras = list(Professional.objects.using(db).filter(loja_id=lid, is_active=True)[:3])
            for p in extras:
                if p not in profissionais:
                    profissionais.append(p)
        email_fixa, email_pct, email_mista = "prof.fixa" + SEED_TAG, "prof.pct" + SEED_TAG, "prof.mista" + SEED_TAG
        ...
        if dr_pct:
            ProfessionalCommission.objects.using(db).filter(professional=dr_pct, tipo="consulta", loja_id=lid).delete()
            for idx, local in enumerate(locais):
                modo, valor = ("percentual", Decimal(str(20 + (idx % 6) * 5))) if idx % 2 == 0 else ("fixo", Decimal(str(80 + idx * 12)))
                self._upsert_comissao(db, lid, dr_pct, "consulta", None, modo, valor, local)
                self.stdout.write(f"   \u2022 {dr_pct.nome} @ {local.nome}: {modo} {valor}")
        return profissionais, dr_pct

    def _seed_procedimentos(self, db, lid, profissionais, dr_pct, models):
        from clinica_beleza.procedimentos_catalogo import procedimento_catalogo_defaults
        Procedure = models["Procedure"]
        ProfessionalCommission = models["ProfessionalCommission"]
        proc_ids_existentes = list(Procedure.objects.using(db).filter(loja_id=lid, descricao=PROC_DESC).values_list("id", flat=True))
        if proc_ids_existentes:
            ProfessionalCommission.objects.using(db).filter(loja_id=lid, tipo="procedimento", procedure_id__in=proc_ids_existentes).delete()
        ...
        for i, item in enumerate(PROCEDIMENTOS_CATALOGO):
            ...
            proc, _ = Procedure.objects.using(db).update_or_create(nome=item.nome, loja_id=lid, defaults=defaults)
            procedimentos.append(proc)
            if dr_pct and i < 10:
                prof = dr_pct
                modo, valor = ("percentual", Decimal(str(15 + (i % 5) * 5))) if i % 2 == 0 else ("fixo", Decimal(str(45 + i * 12)))
            else:
                prof = profissionais[(i // 3) % len(profissionais)]
                modo, valor = ("percentual", Decimal(str(10 + (i % 6) * 5))) if i < 15 else ("fixo", Decimal(str(25 + (i % 8) * 10)))
            self._upsert_comissao(db, lid, prof, "procedimento", proc, modo, valor)
            self.stdout.write(f"   \u2022 {proc.nome}: R$ {proc.preco} \u2014 {prof.nome}: {modo} {valor}")
        return procedimentos

    def _seed_consultas(self, db, lid, profissionais, dr_pct, locais, procedimentos, models, options):
        ...
        for i, (nome_pac, tel, cpf) in enumerate(PACIENTES_CATALOGO):
            ...
            pac, _ = Patient.objects.using(db).update_or_create(
                email=email, loja_id=lid,
                defaults={"nome": nome_pac, "telefone": tel, "cpf": cpf, "is_active": True},
            )
        ...
        for i in range(10):
            self._criar_atendimento(db, lid, i + 1, horas[i], prof_dez_locais, pacientes[i % len(pacientes)], locais[i], _procedimentos_para_consulta(i, procedimentos), hoje, inicio_mes)
    ...
    def handle(self, *args, **options):
        ...
        models = {
            "Appointment": Appointment, "AppointmentProcedure": AppointmentProcedure,
            "Consulta": Consulta, "HorarioTrabalhoProfissional": HorarioTrabalhoProfissional,
            "LocalAtendimento": LocalAtendimento, "Patient": Patient, "Payment": Payment,
            "Procedure": Procedure, "Professional": Professional, "ProfessionalCommission": ProfessionalCommission,
        }
        ...
        for nome, valor, tempo in LOCAIS_CATALOGO:
            local, _ = LocalAtendimento.objects.using(db).update_or_create(
                ...
                defaults={"valor_consulta": valor, "tempo_consulta_minutos": tempo, "is_active": True},
            )
            locais.append(local)
        ...
        profissionais, dr_pct = self._seed_profissionais(db, lid, locais, models)
        procedimentos = self._seed_procedimentos(db, lid, profissionais, dr_pct, models)
        self._seed_consultas(db, lid, profissionais, dr_pct, locais, procedimentos, models, options)
