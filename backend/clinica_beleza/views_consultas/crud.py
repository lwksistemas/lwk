...
from .helpers import ORDEM_FILA_INICIAR, get_consulta_or_404, q_consultas_aguardando_inicio
...
class ConsultaListView(APIView):
    ...
    def get(self, request):
        ...
        if (request.query_params.get("fila") or "").strip().lower() == "iniciar":
            qs = qs.filter(q_consultas_aguardando_inicio()).order_by(*ORDEM_FILA_INICIAR)
